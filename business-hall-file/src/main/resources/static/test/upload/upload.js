...
;(function () {
    ...
    yzFc.initUploader=function () {
        ...
        var uploaderParam = "businessEnumType=22&tableType=2&richEditType=4";
        yzFc.uploader = WebUploader.create({
            ...
            //server: 'http://58.51.90.231:8085/common-file/api/fileUpload/singleImg',
            server: getFileUrl + "imgUpload/singleImg?" + uploaderParam,
        ...
        yzFc.uploader.on( 'fileQueued', function( file ) {
            $("#" + fileListId).append( '<div id="' + file.id + '" class="item">' +
        ...
        yzFc.uploader.on( 'uploadSuccess', function( file,response ) {
            ...
            console.info(response.data.filePath);

            var $li = $(
                '<div id="' + file.id + '" class="file-item">' +
                '<img width="280" height="180" src='+response.data.filePath+'>' +
                '</div>'
            );
